        ...
        <div class="">
            <div class="page-title">        
              <div class="title_left">
                   <h3>VALIDASI DATA PEMINJAMAN PUSAT</h3>
                                        ...
                                        <table id="myTable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                                <thead>
                                                        <tr>
                                                            ...
                                                            <th class="text-center">Verifikasi Peminjaman Pusat</th>           
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                    ...
                                                    if($query->jenis_TA == 'LTA' || $query->jenis_TA == 'SKRIPSI') { ?>
                                                        <?php if($query->validasi_TA == '1' && $query->validasi_jurnal == '1' && $query->validasi_peminjaman_unit == '1') { ?>
                                                                <td class="text-center">   
                                                                    <form action="<?php echo base_url('Backend/Validasi_pusat/lengkap_pusat/'.$data['npm']); ?>" method="post"> 
                                                        ...
                                                        <?php  } else{ ?>
                                                            <td class="text-center">
                                                                <p class="text-danger"> Maaf validasi ini akan ditampilkan jika semua file lainya sudah divalidasi</p>
                                                            </td>
                                                       <?php } ?>
                                                    <?php } elseif($query->jenis_TA == 'TESIS' || $query->jenis_TA == 'DISERTASI') { ?>
                                                        <?php if($query->validasi_TA == '1' && $query->validasi_jurnal == '1' && $query->validasi_sumbangan == '1' && $query->validasi_peminjaman_unit == '1'){?>
                                                            <td class="text-center">   
                                                                <form action="<?php echo base_url('Backend/Validasi_pusat/lengkap_pusat/'.$data['npm']); ?>" method="post"> 
                                                                            <button type="submit" class="btn btn-xs btn-info _btn" data-toggle="tooltip"  title="Lengkap" >
                                                                                <i class="fa fa-fw  fa-check"></i>                                                                          
                        ...
                        <?php foreach($data_mhs as $data) { ?> 
                            <div class="modal fade" id="block<?= $data['npm'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        ...
                                        <div class="modal-body">
                                            <form action="<?= base_url('Backend/Validasi_pusat/belum_lengkap_pusat'); ?>" method="post" >
                                                <?= csrf_field(); ?>
